...
@Service
public class JsonFileData4 {

	public JsonFileData4() {
		super();
...
// Creation du dossier medical par personne
		...
		personsList.forEach(p -> {
				...
				try {
					...
					/*
					 * System.out.println("adresse station");
				...
				/*
				 * System.out.println("Numero de Caserne");
				 * System.out.println(p.getFireStation().getStationNumber());
				 */
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				...
				+ " Date de naissance : " + p.getBirthDate() + " agée de : " + p.age() +" est elle mineure? : " + p.isMinor() +" Dossier Medicale : Medications :"
				+ p.getMedicalRecord().getMedications() + "Allergies  :" + p.getMedicalRecord().getAllergies()
				+ " Depend de la caserne :  " + p.getFireStation().getStationNumber());
});
